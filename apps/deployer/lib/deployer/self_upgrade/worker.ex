defmodule Deployer.SelfUpgrade.Worker do
  ...
  defstruct policy: :hot_only, interval_ms: 60_000, last_failed: nil, timer: nil
  ...
  def init(opts) do
    state = %__MODULE__{
      policy: Keyword.get(opts, :policy, :hot_only),
  ...
  defp upgrade(version, state) do
    ...
    case Executor.hot_upgrade(version) do
      ...
      {:error, reason} when state.policy == :allow_restart ->
        restart(version, reason, state)
  ...
  defp restart(version, hot_reason, state) do
    Logger.warning(
      "Self-upgrade: hot upgrade to #{version} failed (#{inspect(hot_reason)}); restart fallback."
    )

    case Executor.restart_update(version) do
      :ok ->
        emit(:restart_ok, %{version: version})
        {:ok, %{state | last_failed: nil}}

      {:error, reason} ->
        emit(:restart_failed, %{version: version, reason: reason})
        {{:error, reason}, %{state | last_failed: version}}
    end
  end
